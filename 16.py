...
model.fit(xDataRandom,yTrainDataRandom,epochs=1000,batch_size=64,verbose=2)#batch_size略微调大来让训练速度稍微加快

#生成测试数据
xTestData=np.array([[4,5,3,7],[2,1,2,6],[9,8,7,6],[0,1,9,3],[3,3,0,3]],dtype=np.float32)

for i in range(len(xTestData)):
    resultAry=model.predict(np.reshape(xTestData[i],(1,4)))#预测通过调用Keras模型的predict函数进行
